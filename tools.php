    ...
    if ($q['type'] == 'image') return FALSE;
        ...
        if (!$graded) {
            ...
            if (isset($sobj[$slug])) {
                ...
                if ($review !== FALSE && $resp && !isset($sobj[$slug]['grade'])) {
...
function showQuestion($q, $quizid, $qnum, $user, $comments=false, $seeabove=false, $replied=array(), $disable=false, $hist=false, $ajax=true, $unshuffle=false, $default_ordering='shuffle'){
    ...
    if ($q['type'] == 'radio' || $q['type'] == 'checkbox') {
        ...
        foreach($q['options'] as $opt) {
            ...
            if ($hist) {
                ...
                echo "</div>";
                if ($opt['points'] >= 1 || ($q['type'] == 'checkbox' && $opt['points'] >= 0))
    ...
    if ($comments && $ajax && (!$disable || isset($replied['comments']) && $replied['comments'])) {
        echo "<div class='tinput'><span>Comments:</span><textarea ";
        if (!disable) echo "id='comments$qnum' onchange='$postcall' onkeydown='pending($qnum)' onblur='$postcall'";
